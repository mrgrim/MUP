...
public final class RedstoneWireTurbo {
     ...
     */
    private final static class UpdateNode {
        public enum Type {
        ...
        while (updateQueue0.size()>0 || updateQueue1.size()>0) {
            ...
            for (UpdateNode upd : thisLayer) {
                if (upd.type == UpdateNode.Type.REDSTONE) {
                    ...
                    // We don't use World.notifyNeighborsOfStateChange here, since we are
     ...
     */
    public final IBlockState updateSurroundingRedstone(final World worldIn, final BlockPos pos, final IBlockState state, final BlockPos source)
    {
    ...
    private IBlockState calculateCurrentChanges(final World worldIn, final UpdateNode upd)
    {
        ...
        int j;
        //j = RedstoneWireTurbo.getMaxCurrentStrength(upd, j); - not actually used? - MrGrim
        int l = 0;
