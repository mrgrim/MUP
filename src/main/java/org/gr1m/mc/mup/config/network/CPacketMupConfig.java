...
public class CPacketMupConfig implements IMessage
{
    private HashMap<String, Boolean> configList = new HashMap<>();

    public CPacketMupConfig()
    {
    ...
    public void fromBytes(final ByteBuf buf)
    {
        ...
        for (int configIndex = 0; configIndex < configCount; configIndex++)
        {
            ...
            {
                Mup.logger.error("CPacketMupConfig.fromBytes: String decoding failure.");
            }
        }
    }
    ...
    public static class Handler implements IMessageHandler<CPacketMupConfig, IMessage>
    {
        @Override
        public IMessage onMessage(final CPacketMupConfig message, final MessageContext ctx)
        {
            NetHandlerPlayServer handler = ctx.getServerHandler();

            handler.player.getServerWorld().addScheduledTask(() ->
                                                          ConfigPacketHandler.handleClientConfigReceived(message, handler)
                                                     );
